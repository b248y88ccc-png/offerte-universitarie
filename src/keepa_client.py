...
Supporta ASIN manuali e multipli termini di ricerca.
"""
...
def verifica_con_keepa(asin):
    ...
    Ritorna i dettagli dell'offerta se valida.
    """
    ...
    # Determina categoria in base al titolo (semplice)
    categoria_key = "studente"
    if any(word in titolo.lower() for word in ["computer", "tablet", "monitor", "ssd", "mouse", "tastiera"]):
        categoria_key = "tech"
    elif any(word in titolo.lower() for word in ["frigo", "cuscino", "plaid", "coperta", "organizzatore"]):
        categoria_key = "casa"
    
    categoria_info = config.CATEGORIE.get(categoria_key, config.CATEGORIE["studente"])
...
def trova_offerte_su_amazon():
    ...
    # Raccogli tutti i termini di ricerca da tutte le categorie
    tutti_i_termini = []
    for categoria_key, categoria_info in config.CATEGORIE.items():
        termini = categoria_info.get("termini_ricerca", [])
        tutti_i_termini.extend(termini)
    
    # Rimuovi duplicati
    tutti_i_termini = list(set(tutti_i_termini))
    
    # Limita il numero di termini
    termini_da_usare = tutti_i_termini[:config.MAX_TERMINI_RICERCA]
    
    tutte_le_offerte = []
    asins_da_verificare = []
    
    # 1. Aggiungi ASIN manuali dalla configurazione
    ...
    # 2. Scraping per ogni termine
    for termine in termini_da_usare:
    ...
    for asin in asins_da_verificare:
        ...
        time.sleep(0.3)  # Rispetta i limiti API
